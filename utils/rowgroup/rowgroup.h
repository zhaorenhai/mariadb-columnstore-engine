...
public:
    ...
    */
    ...
    inline utils::ConstString getLongString(uint32_t colIndex) const;
    inline utils::ConstString getShortString(uint32_t colIndex) const;
...
inline utils::ConstString Row::getShortString(uint32_t colIndex) const
{
...
inline utils::ConstString Row::getLongString(uint32_t colIndex) const
{
    return inStringTable(colIndex) ?
           strings->getConstString(*((uint64_t*) &data[offsets[colIndex]])) :
           getShortString(colIndex);
}
...
inline void Row::colUpdateMariaDBHasher(datatypes::MariaDBHasher &h, uint32_t col) const
{
    switch (getColType(col))
    {
        ...
        case execplan::CalpontSystemCatalog::TEXT:
        {
            ...
            if (UNLIKELY(getColumnWidth(col) > 8))
                h.add(cs, getLongString(col));
            else
                h.add(cs, getShortString(col));
            break;
        }
        default:
            h.add(&my_charset_bin, getShortString(col));
            break;
    }
}
